...
declare -r EXECUTABLE_PROGRAM="prg2"
...
init() {
    # Check if the executable 'prg2' exists
...
run_valgrind() {
    ...
    if [ -f "${EXECUTABLE_PROGRAM}" ]; then
        local file="i-test.txt"
        echo testread \"integration-compulsory/test-00-commands-in.txt\" \"integration-compulsory/test-00-commands-out.txt\" >$file
...
run_perf_tests() {
    ...
    if [ -f "${EXECUTABLE_PROGRAM}" ]; then
        ...
        echo "perftest 20*get_way_coords 5 $first:$last:$step" >>$file
        echo "perftest 20*get_ways_from 5 $first:$last:$step" >>$file
        echo "perftest 20*route_any 5 $first2:$last:$step2" >>$file
        timeout 300 ./${EXECUTABLE_PROGRAM} $file
        rm $file
    else
        echo "prg2 executable could not be found or built."
    fi
}
...
run_opt_perf_tests() {
    ...
    if [ -f "${EXECUTABLE_PROGRAM}" ]; then
        ...
        local step=1000
        echo "random_seed ${RANDOM_SEED}" >$file
        echo "perftest 20*route_shortest_distance 5 $first:$last:$step" >>$file
        echo "perftest 20*route_least_crossroads 5 $first:$last:$step" >>$file
        echo "perftest 20*route_with_cycle 5 $first:$last:$step" >>$file
        echo "perftest 20*trim_ways 5 $first:$last:$step" >>$file
