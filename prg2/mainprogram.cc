...
MainProgram::CmdResult MainProgram::cmd_get_ways_from(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    string xstr = *begin++;
    string ystr = *begin++;
    assert( begin == end && "Impossible number of parameters!");
    ...
    auto ways = ds_.get_ways_from(coord);
    if (ways.empty())
    {
...
bool MainProgram::test_get_ways_from(Stopwatch& stopwatch, std::ostream& output)
{
    if (random_ways_added_ < 1) // Don't do anything if there's no ways
    {
        return true;
    }
    auto coord = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
    stopwatch.start();
    auto result = ds_.get_ways_from(coord);
    stopwatch.stop();
    output << "";
    return true;
}

MainProgram::CmdResult MainProgram::cmd_get_way_coords(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    string idstr = *begin++;
    assert( begin == end && "Impossible number of parameters!");
...
bool MainProgram::test_get_way_coords(Stopwatch& stopwatch, std::ostream& output)
{
    if (random_ways_added_ < 1)
    {
        return true;
    }
...
void MainProgram::add_random_ways(Stopwatch& stopwatch, unsigned int n)
{
...
bool MainProgram::test_route_any(Stopwatch& stopwatch, std::ostream& output)
{
...
bool MainProgram::test_route_shortest_distance(Stopwatch& stopwatch, std::ostream& output)
{
    // Choose two random places
    Coord coord1 = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
    Coord coord2 = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
...
bool MainProgram::test_route_least_crossroads(Stopwatch& stopwatch, std::ostream& output)
{
    // Choose two random places
    Coord coord1 = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
    Coord coord2 = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
...
bool MainProgram::test_route_with_cycle(Stopwatch& stopwatch, std::ostream& output)
{
    // Choose two random places
    Coord coord1 = n_to_coord(random(decltype(random_ways_added_)(0),random_ways_added_));
...
bool MainProgram::test_trim_ways(Stopwatch& stopwatch, std::ostream& output)
{
    stopwatch.start();
    ds_.trim_ways();
    stopwatch.stop();
...
vector<MainProgram::CmdInfo> MainProgram::cmds_ =
{
    ...
    {"random_ways", "number_of_ways_to_add", numx,
     &MainProgram::cmd_random_ways, nullptr },
    {"get_way_coords", "WayID", wayidx, &MainProgram::cmd_get_way_coords, &MainProgram::test_get_way_coords },
    {"get_ways_from", "Coord", coordx, &MainProgram::cmd_get_ways_from, &MainProgram::test_get_ways_from },
    ...
    try {
        ...
        for (unsigned int n : init_ns)
        {
            ...
            for (unsigned int i = 0; i < n / 1000; ++i)
            {
                ...
                add_random_ways(stopwatch, 1000);
            ...
            {
                // TODO check return value
                add_random_ways(stopwatch, n % 1000);
            }
